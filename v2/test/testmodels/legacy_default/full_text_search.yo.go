...
func (fts *FullTextSearch) columnsToPtrs(cols []string) ([]interface{}, error) {
	ret := make([]interface{}, 0, len(cols))
	for _, col := range cols {
		switch col {
		case "ID":
			ret = append(ret, yoDecode(&fts.ID))
		case "Content":
			ret = append(ret, yoDecode(&fts.Content))
		default:
...
func (fts *FullTextSearch) columnsToValues(cols []string) ([]interface{}, error) {
	...
	for _, col := range cols {
		switch col {
		case "ID":
			ret = append(ret, yoEncode(fts.ID))
		case "Content":
			ret = append(ret, yoEncode(fts.Content))
		default:
			return nil, fmt.Errorf("unknown column: %s", col)
		}
	}
...
func newFullTextSearch_Decoder(cols []string) func(*spanner.Row) (*FullTextSearch, error) {
	return func(row *spanner.Row) (*FullTextSearch, error) {
		...
		ptrs, err := fts.columnsToPtrs(cols)
		if err != nil {
			return nil, err
		}
...
// written become NULL.
func (fts *FullTextSearch) Replace(ctx context.Context) *spanner.Mutation {
	values, _ := fts.columnsToValues(FullTextSearchWritableColumns())
	return spanner.Replace("FullTextSearch", FullTextSearchWritableColumns(), values)
}
...
func FindFullTextSearch(ctx context.Context, db YODB, id int64) (*FullTextSearch, error) {
	_key := spanner.Key{yoEncode(id)}
	row, err := db.ReadRow(ctx, "FullTextSearch", _key, FullTextSearchColumns())
	if err != nil {
...
func ReadFullTextSearch(ctx context.Context, db YODB, keys spanner.KeySet) ([]*FullTextSearch, error) {
