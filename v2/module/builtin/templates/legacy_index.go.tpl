...
func Find{{ .LegacyFuncName }}(ctx {{ $.PackageRegistry.Use presetPackages.context "Context" }}, db YODB{{ goParamDefs $.PackageRegistry .Fields true }}) ([]*{{ .Type.Name }}, error) {
{{- else }}
...
{{- if .IsUnique }}
	...
	if err != nil {
		if err == {{ $.PackageRegistry.Use presetPackages.apiIterator "Done" }} {
			return nil, newErrorWithCode({{ $.PackageRegistry.Use presetPackages.gRPCCodes "NotFound" }}, "Find{{ .LegacyFuncName }}", "{{ $table }}", err)
		}
	...
	if err != nil {
		return nil, newErrorWithCode({{ $.PackageRegistry.Use presetPackages.gRPCCodes "Internal" }}, "Find{{ .LegacyFuncName }}", "{{ $table }}", err)
	}
...
//
// Generated from unique index '{{ .IndexName }}'.
func Read{{ .LegacyFuncName }}(ctx {{ $.PackageRegistry.Use presetPackages.context "Context" }}, db YODB, keys {{ $.PackageRegistry.Use presetPackages.goSpanner "KeySet" }}) ([]*{{ .Type.Name }}, error) {
	var res []*{{ .Type.Name }}
    columns := []string{
...
{{- end }}
	...
	if err != nil {
		return nil, newErrorWithCode({{ $.PackageRegistry.Use presetPackages.gRPCCodes "Internal" }}, "Read{{ .LegacyFuncName }}", "{{ $table }}", err)
	}
